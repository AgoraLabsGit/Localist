...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!highlight) {
    return { title: "Place not found · Localist" };
  }
  ...
  return {
    title: `${highlight.title ?? "Place"} · Localist`,
    description: (highlight.short_description as string) ?? "Smart picks in Buenos Aires with Localist.",
    openGraph: {
      title: `${highlight.title ?? "Place"} · Localist`,
      description: (highlight.short_description as string) ?? "Smart picks in Buenos Aires with Localist.",
      url: shareUrl,
    ...
    <div className="min-h-screen bg-surface text-foreground">
      <div className="mx-auto max-w-lg px-4 py-8">
        ...
        <div className="flex flex-wrap items-center gap-2 mt-4">
          ...
          {appleMapsUrl && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center gap-1 px-3 h-[32px] rounded-[10px] text-[14px] font-medium border border-[rgba(148,163,184,0.35)] bg-transparent text-muted-foreground hover:text-foreground"
            >
        ...
        <p className="mt-4 text-center text-xs text-muted-foreground">
          Smart picks in Buenos Aires with Localist
        </p>
      </div>
    </div>
  );
